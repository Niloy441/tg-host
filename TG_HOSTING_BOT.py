...
TOKEN = '8418880987:AAEfYthIPklfl5rDAoHaCASZWL5J_MHIFos'
OWNER_ID = 7038931465
ADMIN_ID = 7038931465
YOUR_USERNAME = '@Niloy6978'
UPDATE_CHANNEL = 'https://t.me/tgbot7382'
...
# --- Animated Message Functions (unchanged) ---
...
def get_file_icon(ext):
    ...
    if ext == 'py':
        ...
        return '☕'
    elif ext in {'c', 'cpp', 'cc', 'cxx', 'rs'}:
        return '⚙️'
    elif ext in {'txt', 'json', 'env', 'yaml', 'yml', 'xml', 'csv', 'md', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx'}:
        return '📄'
...
(user_id, file_name, file_type, upload_time, file_size, runnable)
VALUES (?, ?, ?, ?, ?, ?)''',
                  (user_id, file_name, file_type, datetime.now().isoformat(), file_size, int(runnable)))
        conn.commit()
        conn.close()
...
def attempt_install_pip(module_name, message):
    ...
    if package_name is None:
        return False
    try:
        msg = send_spinner_animation(message.chat.id, f"Installing {e(package_name)}...", duration=2)
        ...
        if result.returncode == 0:
            try:
                bot.edit_message_text(
                    f"✅ <b>Package Installed!</b>\n📦 <code>{e(package_name)}</code> installed successfully!",
                    message.chat.id, msg.message_id, parse_mode='HTML'
                )
            except:
                bot.send_message(message.chat.id, f"✅ Package {e(package_name)} installed!", parse_mode='HTML')
            return True
        else:
            error_msg = result.stderr[:500] if result.stderr else result.stdout[:500]
            try:
                bot.edit_message_text(
                    f"❌ <b>Installation Failed</b>\n<code>{e(error_msg)}</code>",
                    message.chat.id, msg.message_id, parse_mode='HTML'
                )
            except:
                pass
        ...
        bot.send_message(message.chat.id, f"⏱️ Installation timed out for {e(package_name)}")
        return False
    except Exception as e:
...
def attempt_install_npm(module_name, user_folder, message):
    try:
        msg = send_spinner_animation(message.chat.id, f"Installing npm: {e(module_name)}...", duration=2)
...
def run_script_python(script_path, user_id, user_folder, file_name, message_obj, attempt=1):
    ...
    logger.info(f"Running Python script: {script_path} (Attempt {attempt})")
    try:
        if not os.path.exists(script_path):
            bot.send_message(message_obj.chat.id, f"❌ Script '{e(file_name)}' not found!")
            return
        ...
        if check_result.returncode != 0:
            bot.send_message(message_obj.chat.id,
                             f"⚠️ <b>Syntax Error in Script</b>\n<code>{e(check_result.stderr[:500])}</code>",
                             parse_mode='HTML')
            return
        terminal_msg = f"""
╔══════════════════════════════════════╗
...
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{e(file_name[:25])}</code>
║ 🆔 <b>PID:</b> {process.pid}
...
"""
            ...
            log_action(user_id, "SCRIPT_START", f"Started {file_name} (PID: {process.pid})")
        else:
            log_file.close()
            ...
            if match:
                ...
                if attempt_install_pip(module_name, message_obj):
                    time.sleep(1)
                    run_script_python(script_path, user_id, user_folder, file_name, message_obj, attempt + 1)
                    return
            error_msg = f"""
╔══════════════════════════════════════╗
...
def run_script_node(script_path, user_id, user_folder, file_name, message_obj, attempt=1):
    ...
    if attempt > max_attempts:
        ...
        return
    script_key = f"{user_id}_{file_name}"
    logger.info(f"Running Node script: {script_path} (Attempt {attempt})")
    try:
        if not os.path.exists(script_path):
            bot.send_message(message_obj.chat.id, f"❌ Script '{e(file_name)}' not found!")
            return
...
"""
            ...
            if match:
                ...
                if attempt_install_npm(module_name, user_folder, message_obj):
                    time.sleep(1)
                    run_script_node(script_path, user_id, user_folder, file_name, message_obj, attempt + 1)
                    return
            error_msg = f"""
╔══════════════════════════════════════╗
...
def run_generic_process(script_key, cmd, cwd, file_name, user_id, message_obj, lang_name, env=None):
    ...
    try:
        terminal_msg = f"""
╔══════════════════════════════════════╗
║   🚀 <b>𝐄𝐗𝐔 𝐂𝐎𝐃𝐄𝐑: STARTING {e(lang_name.upper())}</b> 🚀║
╠══════════════════════════════════════╣
║ 📄 File: <code>{e(file_name[:25])}</code>
║ 👤 User: {user_id}
╚══════════════════════════════════════╝
"""
        ...
        bot_scripts[script_key] = {
            ...
            'log_path': log_file_path,
            'script_key': script_key,
            'script_path': os.path.join(cwd, file_name),
            'type': lang_name
        }
        time.sleep(2)
        if process.poll() is None:
            success_msg = f"""
╔══════════════════════════════════════╗
║     ✅ <b>𝐄𝐗𝐔 𝐂𝐎𝐃𝐄𝐑: {e(lang_name.upper())} RUNNING</b> ✅ ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{e(file_name[:25])}</code>
║ 🆔 <b>PID:</b> {process.pid}
║ ⏱️ <b>Started:</b> {datetime.now().strftime('%H:%M:%S')}
...
"""
            ...
            error_msg = f"""
╔══════════════════════════════════════╗
║   ❌ <b>𝐄𝐗𝐔 𝐂𝐎𝐃𝐄𝐑: {e(lang_name.upper())} FAILED</b> ❌  ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{e(file_name[:25])}</code>
║ ❗ <b>Exit Code:</b> {process.returncode}
╠══════════════════════════════════════╣
...
"""
        ...
        logger.error(f"Error running {lang_name}: {e}", exc_info=True)
        bot.send_message(message_obj.chat.id, f"❌ Error: {e(str(e)[:200])}")
...
def compile_and_run(script_key, compile_cmd, run_cmd, cwd, file_name, user_id, message_obj, lang_name):
    ...
    try:
        ...
        if result.returncode != 0:
            try:
                bot.edit_message_text(
                    f"❌ <b>Compilation failed</b>\n<code>{e(result.stderr[:500])}</code>",
                    message_obj.chat.id, compile_msg.message_id, parse_mode='HTML'
                )
            except:
                bot.send_message(message_obj.chat.id, f"❌ Compilation failed: {e(result.stderr[:300])}")
            return
    except subprocess.TimeoutExpired:
        bot.send_message(message_obj.chat.id, "⏱️ Compilation timed out.")
        return
    except Exception as e:
...
def run_script_java(script_path, user_id, user_folder, file_name, message_obj):
    ...
    compile_msg = bot.send_message(message_obj.chat.id,
                                   f"⚙️ 𝐋ᴏᴀᴅɪɴɢ... (0%)\n[⬜⬜⬜⬜] Compiling {e(file_name)}...")
    try:
        result = subprocess.run(['javac', script_path], cwd=user_folder, capture_output=True, text=True, timeout=30)
        if result.returncode != 0:
            bot.edit_message_text(
                f"❌ <b>Compilation failed</b>\n<code>{e(result.stderr[:500])}</code>",
                message_obj.chat.id, compile_msg.message_id, parse_mode='HTML'
        ...
        bot.send_message(message_obj.chat.id, f"❌ Java error: {e(str(e)[:200])}")

def run_script_go(script_path, user_id, user_folder, file_name, message_obj):
    if not shutil.which('go'):
        bot.send_message(message_obj.chat.id, "❌ Go runtime not found. Please install Go.")
        return
    run_generic_process(f"{user_id}_{file_name}", ['go', 'run', script_path],
                        user_folder, file_name, user_id, message_obj, 'Go')

def run_script_ts(script_path, user_id, user_folder, file_name, message_obj):
    if shutil.which('ts-node'):
        run_generic_process(f"{user_id}_{file_name}", ['ts-node', script_path],
                            user_folder, file_name, user_id, message_obj, 'TypeScript')
    else:
        bot.send_message(message_obj.chat.id,
                         "❌ ts-node not found. Please install: npm install -g ts-node typescript")

def run_script_c(script_path, user_id, user_folder, file_name, message_obj):
    out_bin = os.path.splitext(file_name)[0] + '.out'
    compile_and_run(f"{user_id}_{file_name}",
                    ['gcc', script_path, '-o', out_bin],
                    [os.path.join(user_folder, out_bin)],
                    user_folder, file_name, user_id, message_obj, 'C')

def run_script_cpp(script_path, user_id, user_folder, file_name, message_obj):
    out_bin = os.path.splitext(file_name)[0] + '.out'
    compile_and_run(f"{user_id}_{file_name}",
                    ['g++', script_path, '-o', out_bin],
                    [os.path.join(user_folder, out_bin)],
                    user_folder, file_name, user_id, message_obj, 'C++')

def run_script_rust(script_path, user_id, user_folder, file_name, message_obj):
    out_bin = os.path.splitext(file_name)[0] + '.out'
    compile_and_run(f"{user_id}_{file_name}",
                    ['rustc', script_path, '-o', out_bin],
                    [os.path.join(user_folder, out_bin)],
                    user_folder, file_name, user_id, message_obj, 'Rust')

# Dispatcher
def run_user_file(script_path, user_id, user_folder, file_name, message_obj, ext):
    ext = ext.lower()
    if ext == 'py':
        threading.Thread(target=run_script_python,
                         args=(script_path, user_id, user_folder, file_name, message_obj)).start()
    elif ext in ('js', 'mjs', 'cjs'):
        threading.Thread(target=run_script_node,
                         args=(script_path, user_id, user_folder, file_name, message_obj)).start()
    elif ext == 'sh':
        threading.Thread(target=run_generic_process,
                         args=(f"{user_id}_{file_name}", ['bash', script_path],
                               user_folder, file_name, user_id, message_obj, 'Shell')).start()
    elif ext == 'php':
        threading.Thread(target=run_generic_process,
                         args=(f"{user_id}_{file_name}", ['php', script_path],
                               user_folder, file_name, user_id, message_obj, 'PHP')).start()
    elif ext == 'rb':
        threading.Thread(target=run_generic_process,
                         args=(f"{user_id}_{file_name}", ['ruby', script_path],
                               user_folder, file_name, user_id, message_obj, 'Ruby')).start()
    elif ext == 'go':
        threading.Thread(target=run_script_go,
                         args=(script_path, user_id, user_folder, file_name, message_obj)).start()
    elif ext == 'java':
        threading.Thread(target=run_script_java,
                         args=(script_path, user_id, user_folder, file_name, message_obj)).start()
    elif ext == 'c':
        threading.Thread(target=run_script_c,
                         args=(script_path, user_id, user_folder, file_name, message_obj)).start()
    elif ext in ('cpp', 'cc', 'cxx'):
        threading.Thread(target=run_script_cpp,
                         args=(script_path, user_id, user_folder, file_name, message_obj)).start()
    elif ext == 'ts':
        threading.Thread(target=run_script_ts,
                         args=(script_path, user_id, user_folder, file_name, message_obj)).start()
    elif ext == 'lua':
        threading.Thread(target=run_generic_process,
                         args=(f"{user_id}_{file_name}", ['lua', script_path],
                               user_folder, file_name, user_id, message_obj, 'Lua')).start()
    elif ext == 'pl':
        threading.Thread(target=run_generic_process,
                         args=(f"{user_id}_{file_name}", ['perl', script_path],
                               user_folder, file_name, user_id, message_obj, 'Perl')).start()
    elif ext == 'rs':
        threading.Thread(target=run_script_rust,
                         args=(script_path, user_id, user_folder, file_name, message_obj)).start()
    else:
...
╠══════════════════════════════════════╣
║
║  👋 𝐖𝐞𝐥𝐜𝐨𝐦𝐞, <b>{e(message.from_user.first_name)}</b>!
║
...
def speed_command(message):
    msg = send_spinner_animation(message.chat.id, "𝐓𝐞𝐬𝐭𝐢𝐧𝐠 MAHRAB DEV𝐬𝐩𝐞𝐞𝐝...", duration=2)
...
def running_command(message):
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐅𝐞𝐭𝐜𝐡𝐢𝐧𝐠 MAHRAB DEV𝐛𝐨𝐭𝐬...", duration=1)
    running_bots = []
...
"""
        for i, bot_info in enumerate(running_bots, 1):
            text += f"""║ {i}. 📄 <code>{e(bot_info['file'][:20])}</code>
║    👤 𝐔𝐬𝐞𝐫: {bot_info['user']}
║    🆔 𝐏𝐈𝐃: {bot_info['pid']}
...
║
║  𝐒𝐭𝐚𝐭𝐮𝐬: {status}
║  𝐁𝐲: {e(message.from_user.first_name)}
...
def process_broadcast(message):
    ...
    if not broadcast_text:
        ...
        return
    progress_msg = bot.send_message(message.chat.id, "📢 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 MAHRAB DEV𝐛𝐫𝐨𝐚𝐝𝐜𝐚𝐬𝐭...")
    success = 0
    failed = 0
...
def handle_text(message):
    ...
    if text == "📢 Updates Channel":
        bot.send_message(message.chat.id, f"📢 𝐉𝐨𝐢𝐧 𝐨𝐮𝐫 MAHRAB DEV𝐮𝐩𝐝𝐚𝐭𝐞𝐬:\n{UPDATE_CHANNEL}")
...
║  • Executable scripts get a ▶️ Run button
║  • ZIP archives extracted safely
║
...
def show_user_files(message):
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐋𝐨𝐚𝐝𝐢𝐧𝐠 MAHRAB DEV𝐟𝐢𝐥𝐞𝐬...", duration=1)
...
"""
    ...
    for i, (file_name, file_type) in enumerate(files, 1):
        ...
        text += f"║ {i}. {status} {icon} <code>{e(file_name[:25])}</code>\n"
        markup.add(types.InlineKeyboardButton(
            f"{status} {e(file_name[:15])}",
                ...
                with zipfile.ZipFile(tmp_path, 'r') as zip_ref:
                    for member in zip_ref.infolist():
                        ...
                        if member.is_dir():
                            ...
                            user_files[user_id].append((member_name, ext))
                            save_user_file_db(user_id, member_name, ext, 0, runnable)
...
"""
            ...
            user_files[user_id].append((safe_name, file_ext))
            save_user_file_db(user_id, safe_name, file_ext, file_size, runnable)
...
"""
        ...
        if not runnable:
            markup.add(types.InlineKeyboardButton("📥 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝", callback_data=f"download_{safe_name}"))
            markup.add(types.InlineKeyboardButton("🗑️ 𝐃𝐞𝐥𝐞𝐭𝐞", callback_data=f"delete_{safe_name}"))
        try:
        ...
        try:
            bot.edit_message_text(
                upload_text + f"║  ❌ 𝐄𝐫𝐫𝐨𝐫: {e(str(e)[:30])}\n╚══════════════════════════════════════╝",
                message.chat.id, progress_msg.message_id, parse_mode='HTML'
            )
        except:
...
def handle_callback(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id, f"❌ 𝐄𝐫𝐫𝐨𝐫: {e(str(e)[:50])}")
...
def show_file_actions(call, file_name):
    ...
    for name, ftype in user_files.get(user_id, []):
        if name == file_name:
            file_type = ftype
            runnable = is_runnable_extension(ftype)
            break
    icon = get_file_icon(file_type)
...
╠══════════════════════════════════════╣
║
║  {icon} <b>𝐍𝐚𝐦𝐞:</b> <code>{e(file_name[:25])}</code>
║  📁 <b>𝐓𝐲𝐩𝐞:</b> {e(file_type.upper())}
║  📊 <b>𝐒𝐭𝐚𝐭𝐮𝐬:</b> {status}
║
╚══════════════════════════════════════╝
"""
    markup = get_file_actions_keyboard(file_name, is_running, runnable)
    try:
...
"""
        ...
        bot.answer_callback_query(call.id, f"❌ 𝐄𝐫𝐫𝐨𝐫: {e(str(e)[:30])}")
...
def download_user_file(call, file_name):
    ...
    try:
        with open(file_path, 'rb') as f:
            bot.send_document(call.message.chat.id, f, caption=f"📄 {e(file_name)}")
    except Exception as e:
        bot.send_message(call.message.chat.id, f"❌ 𝐄𝐫𝐫𝐨𝐫: {e(str(e)[:100])}")
...
def stop_all_bots(call):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        ...
        return
    bot.answer_callback_query(call.id, "🛑 𝐒𝐭𝐨𝐩𝐩𝐢𝐧𝐠 𝐚𝐥𝐥 MAHRAB DEV𝐛𝐨𝐭𝐬...")
    stopped = 0
...
def main():
    logger.info("=" * 50)
    logger.info("🤖 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 MAHRAB DEV🦁 𝐁𝐨𝐭...")
